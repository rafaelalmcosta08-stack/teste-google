...
const NotificationContext = createContext<NotificationContextType>({
  counts: { avisos: 0, cursos: 0, editais: 0, chat_apm: 0, chat_alto_comando: 0, total: 0 },
  notifications: [],
  refresh: async () => {},
  markAsRead: async () => {},
  clearAll: () => {},
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    if (!user || !profile || !session?.access_token) {
      setCounts({ avisos: 0, cursos: 0, editais: 0, chat_apm: 0, chat_alto_comando: 0, total: 0 })
      setNotifications([])
      return
    }
    ...
    try {
      ...
      // 1. Fetch Avisos, Cursos, Editais in parallel
      const [avisosRes, cursosRes, editaisRes] = await Promise.all([
        fetch('/api/avisos', { headers }),
        fetch('/api/cursos', { headers }),
        fetch('/api/editais', { headers }),
      ])

      const avisosData = avisosRes.ok ? await avisosRes.json() : { avisos: [] }
      const cursosData = cursosRes.ok ? await cursosRes.json() : { courses: [] }
      const editaisData = editaisRes.ok ? await editaisRes.json() : { editais: [] }

      const rawAvisos = avisosData.avisos || []
      const rawCursos = cursosData.courses || []
      const rawEditais = editaisData.editais || []

      // Read arrays of already viewed courses and editais from localStorage
      ...
      // 2. Fetch Chat messages for APM and Alto Comando if authorized
      ...
      unreadCursos.forEach((cur: any) => {
        // Urgent if deadline is less than 24 hours away
      ...
      setCounts({
        ...
        total: unreadAvisos.length + unreadCursos.length + unreadEditais.length + chatApmUnread + chatAltoComandoUnread,
      })
    } catch (err) {
